...
/* Resize the bitset. Return true in case of success, false for failure. Pad with zeroes new buffer
 * areas if requested. */
...
/* Grow the bitset so that it can support newarraysize * 64 bits with padding. Return true in case
 * of success, false for failure. */
...
/* shifts all bits by 's' positions so that the bitset representing values 1,2,10 would represent
 * values 1-s, 2-s, 10-s, negative values are deleted */
...
/* Set the ith bit to the specified value. Attempts to resize the bitset if needed (may silently
 * fail) */
...
/* compute the union in-place (to b1), returns true if successful, to generate a new bitset first
 * call bitset_copy */
...
/* compute the symmetric difference in-place (to b1), return true if successful, to generate a new
 * bitset first call bitset_copy */
...
/* iterate over the set bits
  ...
  */
static inline size_t nextSetBits(const bitset_t *bitset, size_t *buffer, size_t capacity,
                                 size_t *startfrom)
{
    if (capacity == 0)
